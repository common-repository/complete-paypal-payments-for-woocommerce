 ...
 */
class Complete_PayPal_Payments_WooCommerce_Product extends WC_Form_Handler {

    public static function cpp_add_to_cart_action($url = null) {
        try {
        ...
        try {
            ...
            if (!empty($items)) {
                ...
                if (!$was_added_to_cart && !$quantity_set) {
                    wc_add_notice(__('Please choose the quantity of items you wish to add to your cart&hellip;', 'complete-paypal-payments-for-woocommerce'), 'error');
                ...
                wc_add_notice(__('Please choose a product to add to your cart&hellip;', 'complete-paypal-payments-for-woocommerce'), 'error');
            }
            return false;
        } catch (Exception $ex) {
    ...
    private static function add_to_cart_handler_variable($product_id) {
        try {
            ...
            if (empty($variation_id)) {
                throw new Exception(__('Please choose product options&hellip;', 'complete-paypal-payments-for-woocommerce'));
            }
            ...
            foreach ($adding_to_cart->get_attributes() as $attribute) {
                ...
                if (isset($posted_attributes[$attribute_key])) {
                    ...
                    if ($valid_value === $value) {
                        ...
                        throw new Exception(sprintf(__('Invalid value posted for %s', 'complete-paypal-payments-for-woocommerce'), wc_attribute_label($attribute['name'])));
                    }
                } elseif ('' === $valid_value) {
            ...
            if (!empty($missing_attributes)) {
                throw new Exception(sprintf(_n('%s is a required field', '%s are required fields', count($missing_attributes), 'complete-paypal-payments-for-woocommerce'), wc_format_list_of_items($missing_attributes)));
            }
        } catch (Exception $e) {
