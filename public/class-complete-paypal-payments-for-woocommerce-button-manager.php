...
/**
 * @package    Complete_PayPal_Payments_WooCommerce_Button_Manager
 * @subpackage Complete_PayPal_Payments_WooCommerce_Button_Manager/public
 * @author     Eric Babin <completepaypalpayments@gmail.com>
 */
class Complete_PayPal_Payments_WooCommerce_Button_Manager {

    private $plugin_name;
    private $version;
    public $request;
    public $woocommerce_complete_paypal_payments_settings;
    ...
    public function __construct() {
        ...
        if (empty($this->woocommerce_complete_paypal_payments_settings)) {
            $this->woocommerce_complete_paypal_payments_settings = get_option('woocommerce_complete_paypal_payments_settings', true);
        }

        $this->get_properties();
        if ($this->is_valid_for_use() === true) {
            if (!has_action('woocommerce_api_' . strtolower('Complete_PayPal_Payments_WooCommerce_Button_Manager'))) {
                add_action('woocommerce_api_' . strtolower('Complete_PayPal_Payments_WooCommerce_Button_Manager'), array($this, 'handle_wc_api'));
            }
            include_once COMPLETE_PAYPAL_PAYMENTS_FOR_WOOCOMMERCE_PATH . '/includes/class-complete-paypal-payments-for-woocommerce-request.php';
    ...
    public function get_properties() {
        ...
        $this->enabled = 'yes' === $this->cpp_get_settings('enabled', 'no');
        $this->testmode = 'yes' === $this->cpp_get_settings('testmode', 'no');
        if ($this->testmode) {
            $this->client_id = $this->cpp_get_settings('sandbox_client_id');
            $this->secret = $this->cpp_get_settings('sandbox_api_secret');
            $this->webhook_id = 'cpp_sandbox_webhook_id';
        } else {
            $this->client_id = $this->cpp_get_settings('api_client_id');
            $this->secret = $this->cpp_get_settings('api_secret');
            $this->webhook_id = 'cpp_live_webhook_id';
        }
        $this->cpp_currency_list = array('AUD', 'BRL', 'CAD', 'CZK', 'DKK', 'EUR', 'HKD', 'INR', 'ILS', 'JPY', 'MYR', 'MXN', 'TWD', 'NZD', 'NOK', 'PHP', 'PLN', 'GBP', 'RUB', 'SGD', 'SEK', 'CHF', 'THB', 'USD');
        $this->cpp_currency = in_array(get_woocommerce_currency(), $this->cpp_currency_list) ? get_woocommerce_currency() : 'USD';
        $this->enable_product_button = 'yes' === $this->cpp_get_settings('enable_product_button', 'yes');
        $this->enable_cart_button = 'yes' === $this->cpp_get_settings('enable_cart_button', 'yes');
        $this->cart_button_top_position = 'yes' === $this->cpp_get_settings('cart_button_top_position', 'no');
        $this->enable_checkout_button_top = 'yes' === $this->cpp_get_settings('enable_checkout_button_top', 'no');
        $this->enable_checkout_button = 'yes' === $this->cpp_get_settings('enable_checkout_button', 'yes');
        $this->enable_mini_cart_button = 'yes' === $this->cpp_get_settings('enable_mini_cart_button', 'yes');
        $this->order_review_page_title = $this->cpp_get_settings('order_review_page_title', 'Confirm your PayPal order');
        $this->order_review_page_enable_coupons = 'yes' === $this->cpp_get_settings('order_review_page_enable_coupons', 'yes');
        $this->order_review_page_description = $this->cpp_get_settings('order_review_page_description', __("<strong>You're almost done!</strong><br>Review your information before you place your order.", 'complete-paypal-payments-for-woocommerce'));
        $this->paymentaction = $this->cpp_get_settings('paymentaction', 'capture');
        $this->advanced_card_payments = 'yes' === $this->cpp_get_settings('enable_advanced_card_payments', 'no');
        if (cpp_is_advanced_cards_available() === false) {
            $this->advanced_card_payments = false;
        }
        $this->threed_secure_contingency = $this->cpp_get_settings('3d_secure_contingency', 'SCA_WHEN_REQUIRED');
        $this->enabled_pay_later_messaging = 'yes' === $this->cpp_get_settings('enabled_pay_later_messaging', 'no');
        $this->pay_later_messaging_page_type = $this->cpp_get_settings('pay_later_messaging_page_type', array('home', 'category', 'product', 'cart', 'payment'));
        if (empty($this->pay_later_messaging_page_type)) {
            $this->enabled_pay_later_messaging = false;
        }
        $this->set_billing_address = false;
    ...
    public function cpp_add_hooks() {
        ...
        add_filter('clean_url', array($this, 'cpp_clean_url'));
        add_action('wp_loaded', array($this, 'cpp_session_manager'), 999);
        add_action('wp_head', array($this, 'cpp_add_header_meta'), 0);
        add_filter('the_title', array($this, 'cpp_endpoint_page_titles'));
        add_action('woocommerce_cart_emptied', array($this, 'maybe_clear_session_data'));
        add_action('woocommerce_checkout_init', array($this, 'cpp_checkout_init'));
        ...
        add_filter('body_class', array($this, 'cpp_add_class_order_review_page'));
        add_filter('woocommerce_coupons_enabled', array($this, 'cpp_woocommerce_coupons_enabled'), 999, 1);
        add_action('woocommerce_before_checkout_form', array($this, 'cpp_order_review_page_description'), 9);
        add_action('woocommerce_order_status_processing', array($this, 'cpp_capture_payment'));
        add_action('woocommerce_order_status_completed', array($this, 'cpp_capture_payment'));
        add_action('woocommerce_order_status_cancelled', array($this, 'cpp_cancel_authorization'));
        add_action('woocommerce_order_status_refunded', array($this, 'cpp_cancel_authorization'));
        add_filter('woocommerce_order_actions', array($this, 'cpp_add_capture_charge_order_action'));
        add_action('woocommerce_order_action_cpp_capture_charge', array($this, 'cpp_maybe_capture_charge'));
        add_action('woocommerce_before_checkout_form', array($this, 'cpp_update_checkout_field_details'));
        add_action('woocommerce_review_order_before_submit', array($this, 'cpp_cancel_button'));
        add_action('woocommerce_pay_order_after_submit', array($this, 'cpp_add_order_id'));
        add_action('wp_loaded', array($this, 'cpp_prevent_add_to_cart_woo_action'), 1);
    ...
    public function enqueue_scripts() {
        ...
        $this->cpp_smart_button_style_properties();
        $smart_js_arg = array();
        $smart_js_arg['client-id'] = $this->client_id;
        $smart_js_arg['currency'] = $this->cpp_currency;
        ...
        if ($this->testmode) {
            if (defined('WP_DEBUG') && WP_DEBUG && is_user_logged_in() && WC()->customer && WC()->customer->get_billing_country() && 2 === strlen(WC()->customer->get_billing_country())) {
        ...
        if (is_product()) {
            $page = 'product';
            $button_selector['cpp_product_page'] = '#cpp_product';
        } elseif (is_cart() && !WC()->cart->is_empty() && $this->enable_cart_button) {
            $page = 'cart';
            if ($this->cart_button_top_position) {
                $button_selector['cpp_cart'] = '#cpp_cart';
                $button_selector['cpp_cart_top'] = '#cpp_cart_top';
            }
            $button_selector['cpp_cart'] = '#cpp_cart';
        } elseif (is_checkout_pay_page()) {
            $page = 'checkout';
            $button_selector['cpp_checkout'] = '#cpp_checkout';
            $is_pay_page = 'yes';
        } elseif (is_checkout()) {
            $page = 'checkout';
            $button_selector['cpp_checkout'] = '#cpp_checkout';
            $button_selector['cpp_checkout_top'] = '#cpp_checkout_top';
        }
        ...
        wp_register_script($this->plugin_name, COMPLETE_PAYPAL_PAYMENTS_FOR_WOOCOMMERCE_URL . '/public/js/complete-paypal-payments-for-woocommerce-public.js', array('jquery'), time(), false);
        $this->style_layout = is_product() ? 'horizontal' : 'vertical';
        wp_localize_script($this->plugin_name, 'cpp_manager', array(
            ...
            'display_order_page' => add_query_arg(array('cpp_action' => 'display_order_page', 'utm_nooverride' => '1'), WC()->api_request_url('Complete_PayPal_Payments_WooCommerce_Button_Manager')),
            'cc_capture' => add_query_arg(array('cpp_action' => 'cc_capture', 'utm_nooverride' => '1'), WC()->api_request_url('Complete_PayPal_Payments_WooCommerce_Button_Manager')),
            'create_order_url' => add_query_arg(array('cpp_action' => 'create_order', 'utm_nooverride' => '1', 'from' => is_checkout_pay_page() ? 'pay_page' : $page), WC()->api_request_url('Complete_PayPal_Payments_WooCommerce_Button_Manager')),
        ...
        if (is_checkout() && empty($this->checkout_details)) {
            wp_enqueue_script('complete-paypal-payments-for-woocommerce-order-review', COMPLETE_PAYPAL_PAYMENTS_FOR_WOOCOMMERCE_URL . '/public/js/complete-paypal-payments-for-woocommerce-order-review.js', array('jquery'), $this->version, false);
        } elseif (is_checkout() && !empty($this->checkout_details)) {
            wp_enqueue_script('complete-paypal-payments-for-woocommerce-order-review', COMPLETE_PAYPAL_PAYMENTS_FOR_WOOCOMMERCE_URL . '/public/js/complete-paypal-payments-for-woocommerce-order-capture.js', array('jquery'), $this->version, false);
        }
    }

    public function enqueue_styles() {
        wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/complete-paypal-payments-for-woocommerce-public.css', array(), $this->version, 'all');
    }
    ...
    public function display_paypal_button_cart_page() {
        ...
        if (WC()->cart->needs_payment()) {
            ...
            echo '<div class="psb-button-container"><div id="cpp_cart"></div><div class="psb-proceed-to-checkout-button-separator">&mdash; ' . __('OR', 'complete-paypal-payments-for-woocommerce') . ' &mdash;</div></div>';
        }
    }
    ...
    public function display_paypal_button_cart_page_top() {
        ...
        if (WC()->cart->needs_payment()) {
            wp_enqueue_script('psb-checkout-js');
            wp_enqueue_script($this->plugin_name);
            echo '<div class="psb-button-container"><div id="cpp_cart_top"></div></div>';
        }
    }
    ...
    public function display_paypal_button_product_page() {
        ...
        wp_enqueue_script('psb-checkout-js');
        wp_enqueue_script($this->plugin_name);
        echo '<div class="psb-button-container"><div id="cpp_product"></div></div>';
    }
    ...
    public function display_paypal_button_checkout_page() {
        if (has_active_session() === false) {
            wp_enqueue_script('psb-checkout-js');
            wp_enqueue_script($this->plugin_name);
            echo '<div class="psb-button-container"><div id="cpp_checkout"></div><div class="psb-proceed-to-checkout-button-separator checkout_cc_separator" style="display:none;">&mdash;&mdash; ' . __('OR', 'complete-paypal-payments-for-woocommerce') . ' &mdash;&mdash;</div></div>';
        }
    }
    ...
    public function display_paypal_button_top_checkout_page() {
        if (has_active_session() === false) {
            wp_enqueue_script('psb-checkout-js');
            wp_enqueue_script($this->plugin_name);
            echo apply_filters('cpp_checkout_top_html', '<div class="psb-button-container"><fieldset><legend class="express-title">' . __('PayPal Checkout', 'complete-paypal-payments-for-woocommerce') . '</legend><div class="wc_ppcp_express_checkout_gateways"><div class="express_payment_method_ppcp"><div id="cpp_checkout_top"></div></div></div></fieldset><span class="express-divider">OR</span></div>');
        }
    }

    public function cpp_add_header_meta() {
        if ($this->is_valid_for_use() === true) {
    ...
    public function cpp_get_settings($key, $bool = false) {
        if (!empty($this->woocommerce_complete_paypal_payments_settings)) {
            if (!empty($this->woocommerce_complete_paypal_payments_settings[$key])) {
                return $this->woocommerce_complete_paypal_payments_settings[$key];
            } else {
                return $bool;
            }
        } else {
    ...
    public function cpp_endpoint_page_titles($title) {
        if (!is_admin() && is_main_query() && in_the_loop() && is_page() && is_checkout() && !empty($this->checkout_details)) {
            $title = $this->order_review_page_title;
            remove_filter('the_title', array($this, 'cpp_endpoint_page_titles'));
        }
        return $title;
    }

    public function handle_wc_api() {
        if (!empty($_GET['cpp_action'])) {
            switch ($_GET['cpp_action']) {
                case "webhook_handler":
                    $this->cpp_handle_webhook_request();
                    ob_clean();
                ...
                case "create_order":
                    if (isset($_GET['from']) && 'pay_page' === $_GET['from']) {
                        $woo_order_id = wc_clean($_POST['woo_order_id']);
                        ...
                        if (isset($_POST) && !empty($_POST)) {
                            ...
                            $_GET['from'] = 'cart';
                            $this->request->cpp_create_order_request();
                            exit();
                        }
                    } elseif (isset($_GET['from']) && 'product' === $_GET['from']) {
                        try {
                            Complete_PayPal_Payments_WooCommerce_Product::cpp_add_to_cart_action();
                            $this->request->cpp_create_order_request();
                            exit();
                        } catch (Exception $ex) {
                ...
                case "display_order_page":
                    $this->cpp_display_order_page();
                    break;
                case "cc_capture":
                    wc_clear_notices();
                    cpp_set_session('cpp_paypal_order_id', wc_clean($_GET['paypal_order_id']));
                    $this->cpp_cc_capture();
                    break;
            }
        }
    ...
    public function cpp_checkout_init($checkout) {
    ...
    public function cpp_prepare_order_data() {
        if (empty($this->checkout_details)) {
            $this->checkout_details = cpp_get_session('cpp_paypal_transaction_details', false);
            if (empty($this->checkout_details)) {
                $cpp_paypal_order_id = cpp_get_session('cpp_paypal_order_id');
                if (!empty($cpp_paypal_order_id)) {
                    $this->checkout_details = $this->request->cpp_get_checkout_details($cpp_paypal_order_id);
                }
            }
            if (empty($this->checkout_details)) {
                return;
            }
            cpp_set_session('cpp_paypal_transaction_details', $this->checkout_details);
        }
        ...
        $order_data['payment_method'] = 'complete_paypal_payments';
    ...
    public function paypal_billing_details() {
        ...
        ?>
        <div class="cpp_billing_details">
            <?php if (wc_ship_to_billing_address_only() && WC()->cart->needs_shipping()) : ?>
                <h3><?php esc_html_e('Billing &amp; Shipping', 'complete-paypal-payments-for-woocommerce'); ?>&nbsp;&nbsp;&nbsp;<a class="cpp_edit_billing_address"><?php _e('Edit', 'complete-paypal-payments-for-woocommerce'); ?></a></h3>
            <?php else : ?>
                <h3><?php esc_html_e('Billing details', 'complete-paypal-payments-for-woocommerce'); ?>&nbsp;&nbsp;&nbsp;<a class="cpp_edit_billing_address"><?php _e('Edit', 'complete-paypal-payments-for-woocommerce'); ?></a></h3>
            <?php
            endif;
    ...
    public function paypal_shipping_details() {
        ...
        ?>
        <div class="cpp_shipping_details">
            <h3><?php _e('Shipping details', 'complete-paypal-payments-for-woocommerce'); ?>&nbsp;&nbsp;&nbsp;<a class="cpp_edit_shipping_address"><?php _e('Edit', 'complete-paypal-payments-for-woocommerce'); ?></a></h3>
            <?php
        ...
        public function get_mapped_billing_address($checkout_details) {
            ...
            if (!empty($this->checkout_details->payer->phone->phone_number->national_number)) {
                $phone = $this->checkout_details->payer->phone->phone_number->national_number;
            ...
            if (!is_user_logged_in() && $checkout->enable_signup) {
                ...
                if (!empty($checkout->checkout_fields['account'])) {
                    ...
                    <div class="create-account">
                        <p><?php _e('Create an account by entering the information below. If you are a returning customer please login at the top of the page.', 'complete-paypal-payments-for-woocommerce'); ?></p>
        ...
        public function maybe_disable_other_gateways($gateways) {
            if (!is_admin() && is_main_query() && in_the_loop() && is_page() && is_checkout() && !empty($this->checkout_details)) {
                foreach ($gateways as $id => $gateway) {
                    if ('complete_paypal_payments' !== $id) {
                        unset($gateways[$id]);
                    }
                }
            }
            if (is_cart() || ( is_checkout() && !is_checkout_pay_page() )) {
                if (isset($gateways['complete_paypal_payments']) && ( 0 >= WC()->cart->total )) {
                    unset($gateways['complete_paypal_payments']);
                }
            }
            return $gateways;
        }
        ...
        public function copy_checkout_details_to_post() {
            if (empty($this->checkout_details)) {
                ...
                if (empty($this->checkout_details)) {
                    if (!empty($_GET['paypal_order_id'])) {
                        $this->checkout_details = $this->request->cpp_get_checkout_details($_GET['paypal_order_id']);
                    }
                }
            ...
            if (!isset($_POST['payment_method']) || ( 'complete_paypal_payments' !== $_POST['payment_method'] ) || empty($this->checkout_details)) {
                return;
            }
        ...
        public function cpp_session_manager() {
            try {
        ...
        public function cpp_display_order_page() {
            $this->checkout_details = $this->request->cpp_get_checkout_details($_GET['paypal_order_id']);
            cpp_set_session('cpp_paypal_transaction_details', $this->checkout_details);
            if (empty($this->checkout_details)) {
                return false;
            ...
            $order_id = absint(cpp_get_session('order_awaiting_payment'));
            if (empty($order_id)) {
                $order_id = cpp_get_session('cpp_woo_order_id');
            }
            ...
            if ($this->paymentaction === 'capture' && !empty($this->checkout_details->status) && $this->checkout_details->status == 'COMPLETED' && $order !== false) {
                ...
                if ($payment_status == 'COMPLETED') {
                    ...
                    $order->add_order_note(sprintf(__('Payment via %s : %s .', 'complete-paypal-payments-for-woocommerce'), $this->title, ucfirst(strtolower($payment_status))));
                } else {
                    ...
                    cpp_update_woo_order_status($order_id, $payment_status, $payment_status_reason);
                }
                $order->add_order_note(sprintf(__('%s Transaction ID: %s', 'complete-paypal-payments-for-woocommerce'), $this->title, $transaction_id));
                ...
                if (!empty($payment_status_reason)) {
                    $order->add_order_note(sprintf(__('Payment via %s Pending. PayPal reason: %s.', 'complete-paypal-payments-for-woocommerce'), $this->title, $payment_status_reason));
                }
        ...
        public function maybe_start_checkout($data, $errors = null) {
            try {
                ...
                if (empty($error_messages)) {
                    $this->set_customer_data(wc_clean(wp_unslash($_POST)));
                } else {
                    ob_start();
        ...
        public function cpp_add_class_order_review_page($classes) {
            try {
        ...
        public function cpp_woocommerce_coupons_enabled($bool) {
            if (has_active_session()) {
        ...
        public function cpp_order_review_page_description() {
        ...
        public function cpp_capture_payment($order_id) {
            $order = wc_get_order($order_id);
            if (!$order) {
                return;
            }
            ...
            $payment_action = cpp_get_post_meta($order, '_payment_action');
            $auth_transaction_id = cpp_get_post_meta($order, '_auth_transaction_id');
            if ('complete_paypal_payments' === $payment_method && $payment_action === 'authorize' && !empty($auth_transaction_id)) {
                $trans_details = $this->request->cpp_show_details_authorized_payment($auth_transaction_id);
                if ($this->cpp_is_authorized_only($trans_details)) {
                    $this->request->cpp_capture_authorized_payment($order_id);
                }
            }
        }

        public function cpp_cancel_authorization($order_id) {
            $order = wc_get_order($order_id);
            if (!$order) {
                return;
            }
            ...
            $payment_action = cpp_get_post_meta($order, '_payment_action');
            if ('complete_paypal_payments' === $payment_method && $transaction_id && $payment_action === 'authorize') {
                // $trans_details = $this->request->cpp_show_details_authorized_payment($transaction_id);
                //if ($this->cpp_is_authorized_only($trans_details)) {
                //  $this->request->cpp_capture_authorized_payment($order_id);
                //}
            }
        ...
        public function cpp_add_capture_charge_order_action($actions) {
            ...
            $old_wc = version_compare(WC_VERSION, '3.0', '<');
            $payment_method = $old_wc ? $order->payment_method : $order->get_payment_method();
            $paypal_status = cpp_get_post_meta($order, '_payment_status');
            $payment_action = cpp_get_post_meta($order, '_payment_action');
            if ('complete_paypal_payments' !== $payment_method) {
                return $actions;
            }
            if (!is_array($actions)) {
                $actions = array();
            ...
            if ('CREATED' !== $paypal_status && $payment_action === 'authorize') {
                $actions['cpp_capture_charge'] = esc_html__('Capture Charge', 'complete-paypal-payments-for-woocommerce');
            }
            return $actions;
        }

        public function cpp_maybe_capture_charge($order) {
            if (!is_object($order)) {
                $order = wc_get_order($order);
            }
            $order_id = version_compare(WC_VERSION, '3.0', '<') ? $order->id : $order->get_id();
            $this->cpp_capture_payment($order_id);
            return true;
        }

        public function cpp_is_authorized_only($trans_details = array()) {
        ...
        public function cpp_clean_url($url) {
            if (strpos($url, 'https://www.paypal.com/sdk/js') !== false) {
                $client_token = $this->request->cpp_get_generate_token();
        ...
        public function cpp_set_address_to_checkout_field() {
        ...
        public function cpp_update_checkout_field_details() {
            if (empty($this->checkout_details)) {
                $this->checkout_details = cpp_get_session('cpp_paypal_transaction_details', false);
                if (empty($this->checkout_details)) {
                    if (!empty($_GET['paypal_order_id'])) {
                        ...
                        if (!empty($value)) {
                            if ('state' == $field) {
                                if ($this->validate_checkout($shipping_address['country'], $value, 'shipping')) {
                                    $_POST['shipping_' . $field] = wc_clean($this->validate_checkout($shipping_address['country'], $value, 'shipping'));
                                } else {
                                    if (isset($shipping_address['country']) && isset($states_list[$shipping_address['country']])) {
                                        ...
                                        $_POST['shipping_' . $field] = wc_clean($state_key);
                                    } else {
                ...
                if (!empty($billing_address)) {
                    foreach ($billing_address as $field => $value) {
                        if (!empty($value)) {
                            if ('state' == $field) {
                                if ($this->validate_checkout($shipping_address['country'], $value, 'shipping')) {
                                    $_POST['billing_' . $field] = wc_clean($this->validate_checkout($shipping_address['country'], $value, 'shipping'));
                                } else {
                                    if (isset($shipping_address['country']) && isset($states_list[$shipping_address['country']])) {
                                        $state_key = array_search($value, $states_list[$shipping_address['country']]);
                                        $_POST['billing_' . $field] = wc_clean($state_key);
                                    } else {
        ...
        public function cpp_cancel_button() {
            if (has_active_session()) {
                $order_button_text = __('Cancel order', 'complete-paypal-payments-for-woocommerce');
                $cancel_order_url = add_query_arg(array('cpp_action' => 'cancel_order', 'utm_nooverride' => '1', 'from' => 'checkout'), WC()->api_request_url('Complete_PayPal_Payments_WooCommerce_Button_Manager'));
                echo apply_filters('cpp_review_order_cance_button_html', '<a class="button alt cpp_cancel" name="woocommerce_checkout_cancel_order" href="' . esc_attr($cancel_order_url) . '" >' . $order_button_text . '</a>');
            }
        }

        public function cpp_handle_webhook_request() {
            $this->request->cpp_handle_webhook_request_handler();
        }

        public function cpp_cc_capture() {
            try {
                $cpp_paypal_order_id = cpp_get_session('cpp_paypal_order_id');
                if (!empty($cpp_paypal_order_id)) {
                    ...
                    $this->request = new Complete_PayPal_Payments_WooCommerce_Request();
                    $api_response = $this->request->cpp_get_checkout_details($cpp_paypal_order_id);
                    ...
                    $order = wc_get_order($order_id);
                    if ($order === false) {
                        if (!class_exists('Complete_PayPal_Payments_WooCommerce_Checkout')) {
                            include_once COMPLETE_PAYPAL_PAYMENTS_FOR_WOOCOMMERCE_DIR . '/includes/class-complete-paypal-payments-for-woocommerce-checkout.php';
                        }
                        $ppcp_checkout = Complete_PayPal_Payments_WooCommerce_Checkout::instance();
                        $order_id = $ppcp_checkout->ccp_create_order();
                        $order = wc_get_order($order_id);
                        $this->request->cpp_update_order($order);
                    }
                    $liability_shift_result = $this->cpp_liability_shift($order, $api_response);
                    if ($liability_shift_result === 1) {
                        if ($this->paymentaction === 'capture') {
                            $is_success = $this->request->cpp_order_capture_request($order_id, false);
                        } else {
                            $is_success = $this->request->cpp_order_auth_request($order_id);
                        }
                        cpp_update_post_meta($order, '_payment_action', $this->paymentaction);
                        cpp_update_post_meta($order, 'enviorment', ($this->testmode) ? 'sandbox' : 'live');
                    } elseif ($liability_shift_result === 2) {
                        $is_success = false;
                        wc_add_notice(__('We cannot process your order with the payment information that you provided. Please use an alternate payment method.', 'complete-paypal-payments-for-woocommerce'), 'error');
                    } elseif ($liability_shift_result === 3) {
                        $is_success = false;
                        wc_add_notice(__('Something went wrong. Please try again.', 'complete-paypal-payments-for-woocommerce'), 'error');
                    }
                    if ($is_success) {
                        WC()->cart->empty_cart();
                        unset(WC()->session->cpp_session);
        ...
        public function cpp_liability_shift($order, $response_object) {
            if (!empty($response_object)) {
                ...
                if (!empty($response['payment_source']['card']['authentication_result']['liability_shift'])) {
                    ...
                    $liability_shift_order_note = __('3D Secure response', 'complete-paypal-payments-for-woocommerce');
                    $liability_shift_order_note .= "\n";
                    $liability_shift_order_note .= 'Liability Shift : ' . cpp_readable($LiabilityShift);
                    $liability_shift_order_note .= "\n";
        ...
        public function cpp_add_order_id() {
            global $wp;
        ...
        public function cpp_smart_button_style_properties() {
            $this->disable_funding = array();
            $this->style_layout = $this->cpp_get_settings('cart_button_layout', 'vertical');
            $this->style_color = 'gold';
            $this->style_shape = 'rect';
            $this->style_label = 'paypal';
            $this->style_tagline = 'yes';
            if (is_product()) {
                $this->disable_funding = $this->cpp_get_settings('product_disallowed_funding_methods', array());
                $this->style_layout = $this->cpp_get_settings('product_button_layout', 'horizontal');
                $this->style_color = $this->cpp_get_settings('product_style_color', 'gold');
                $this->style_shape = $this->cpp_get_settings('product_style_shape', 'rect');
                $this->style_label = $this->cpp_get_settings('product_button_label', 'paypal');
                $this->style_tagline = $this->cpp_get_settings('product_button_tagline', 'yes');
            } elseif (is_cart()) {
                $this->disable_funding = $this->cpp_get_settings('cart_disallowed_funding_methods', array());
                $this->style_layout = $this->cpp_get_settings('cart_button_layout', 'vertical');
                $this->style_color = $this->cpp_get_settings('cart_style_color', 'gold');
                $this->style_shape = $this->cpp_get_settings('cart_style_shape', 'rect');
                $this->style_label = $this->cpp_get_settings('cart_button_label', 'paypal');
                $this->style_tagline = $this->cpp_get_settings('cart_button_tagline', 'yes');
            } elseif (is_checkout() || is_checkout_pay_page()) {
                $this->disable_funding = $this->cpp_get_settings('checkout_disallowed_funding_methods', array());
                $this->style_layout = $this->cpp_get_settings('checkout_button_layout', 'vertical');
                $this->style_color = $this->cpp_get_settings('checkout_style_color', 'gold');
                $this->style_shape = $this->cpp_get_settings('checkout_style_shape', 'rect');
                $this->style_label = $this->cpp_get_settings('checkout_button_label', 'paypal');
                $this->style_tagline = $this->cpp_get_settings('checkout_button_tagline', 'yes');
            }
            $this->mini_cart_disable_funding = $this->cpp_get_settings('mini_cart_disallowed_funding_methods', array());
            $this->mini_cart_style_layout = $this->cpp_get_settings('mini_cart_button_layout', 'vertical');
            $this->mini_cart_style_color = $this->cpp_get_settings('mini_cart_style_color', 'gold');
            $this->mini_cart_style_shape = $this->cpp_get_settings('mini_cart_style_shape', 'rect');
            $this->mini_cart_style_label = $this->cpp_get_settings('mini_cart_button_label', 'paypal');
            $this->mini_cart_style_tagline = $this->cpp_get_settings('mini_cart_button_tagline', 'yes');
        }

        public function cpp_prevent_add_to_cart_woo_action() {
